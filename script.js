  ...
  animate = function () {
    ...
    switch (state) {
      ...
      case 20:
        ...
        puzzle.scale();
        puzzle.startTime = Date.now(); // Add start time tracking
        puzzle.polyPieces.forEach(pp => {
          pp.drawImage();
        ...
        switch (event.event) {
          ...
          case "leave":
            ...
            if (puzzle.polyPieces.length === 1) {
              let timeSpent = Math.floor((Date.now() - puzzle.startTime)/1000);
              let totalPieces = puzzle.nx * puzzle.ny;
              ...
              overlay.innerHTML = `
                <div class="completion-card">
                  ...
                  <p class="completion-message">
                    Kamu Berhasil Selesaikan Puzzle-Nya!!!
                  </p>
                  <div class="completion-stats">
                    <p class="stat-item">
                      ✨ Total Potongan: ${totalPieces} pieces
                    </p>
                    <p class="stat-item">
                      ⏱️ Waktu Penyelesaian: ${timeSpent} detik
                    </p>
                  </div>
              ...
              puzzle.container.appendChild(overlay);

              // Add restart button handler
              overlay.querySelector('.restart-button').addEventListener('click', handleRestart);
              overlay.querySelector('.restart-button').addEventListener('touchend', handleRestart);

              function handleRestart(e) {
                e.preventDefault(); // Prevent any default behavior
                e.stopPropagation(); // Stop event bubbling
                overlay.remove();
                events.push({ event: "nbpieces", nbpieces: puzzle.nbPieces });
              }
              ...
              state = 65; // Skip state 60, go directly to waiting for new game
            } else {
              state = 50; // continue playing
            }
            return;
      ...
      case 65: // wait for new number of pieces - of new picture
        if (event && event.event == "nbpieces") {
          // Clear container and restart game
